...
function App(){
  ...
  useEffect(()=>{ const t=setTimeout(()=>setBootLoading(false),1800); api().licenseCached().then((r:any)=>{ const msg=friendly(r); setLicenseText(msg); }).catch(()=>{}); api().machineId().then((r:any)=>{ if(r?.machineId)setMachineId(r.machineId) }).catch(()=>{}); api().status().then(append).catch(()=>{}); return ()=>clearTimeout(t); },[])
      ...
      {page==='ai'&&<div className="grid ai">
        ...
        <Card title="Thiết lập Flow" icon={<Film/>}>
          <div className="form4">
            ...
            <Field label="Số output"><select value={count} onChange={e=>setCount(e.target.value)}>{['1','2','3','4'].map(x=><option key={x} value={x}>{x}</option>)}</select></Field>
          </div>
