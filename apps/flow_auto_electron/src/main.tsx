...
function App(){
  ...
  const [profiles,setProfiles]=useState<any[]>([{name:'Profile 1',script:''},{name:'Profile 2',script:''}]);
  ...
  async function pickRefs(){const r=await api().openFile({properties:['openDirectory']}); if(r?.[0]){setRefsDir(r[0]); append(`Ref folder: ${r[0]}`)}}
  ...
  function runPayload(file?:string){return {promptFile:file||promptFile||generatedFile, mode, model, ratio, count, spacing, refsDir, runMode, flowThreads, autoDownload:true, pairedMode:true, subMode:'frames', referenceMode:'ingredients'}}
    ...
    <main className="main">
      ...
      {page==='multi'&&<div className="multi-page">
        <Card title="Đa luồng profile Flow" icon={<Film/>}>
          ...
          <div className="profile-grid">{profiles.map((pr:any,i:number)=><div className="profile-card" key={i}><Field label={`Tên profile ${i+1}`}><input value={pr.name} onChange={e=>setProfiles(p=>p.map((x,k)=>k===i?{...x,name:e.target.value}:x))}/></Field><Field label="Kịch bản / prompt cho profile này"><textarea value={pr.script} onChange={e=>setProfiles(p=>p.map((x,k)=>k===i?{...x,script:e.target.value}:x))} placeholder="Mỗi đoạn cách nhau 1 dòng trống = 1 prompt"/></Field></div>)}</div>
