      ...
      {page==='ai'&&<div className="grid ai">
        ...
        <Card title="Thiết lập Flow" icon={<Film/>}>
          <div className="form4">
            ...
            <Field label="Số output"><select value={count} onChange={e=>setCount(e.target.value)}>{['1','2','3','4'].map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Chế độ chạy"><select value={runMode} onChange={e=>setRunMode(e.target.value)}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - chỉ submit</option><option value="continuous_download_delay_3">Chạy liên tục - download trễ 3 prompt</option></select></Field><Field label="Giãn cách prompt"><input value={spacing} onChange={e=>setSpacing(e.target.value)}/></Field><Field label="Số tab Flow"><select value={flowThreads} onChange={e=>setFlowThreads(e.target.value)}>{['1','2','3','4','5'].map(x=><option key={x} value={x}>{x} tab</option>)}</select></Field>
          </div>
