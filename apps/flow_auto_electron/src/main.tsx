...
function App(){
  ...
  const nav=[['flow','Vận hành Flow',Film],['ai','AI Prompt Studio',Wand2],['post','Hậu kì video',Scissors],['payment','Thanh toán',CreditCard],['license','License',KeyRound]];
  ...
  return <div className="app">{bootLoading&&<div className="boot-loading"><div className="loader-card"><div className="spinner"></div><b>Đang tải ứng dụng...</b><span>FLOW AUTO VEO 3 đang khởi động, vui lòng chờ.</span></div></div>}
    ...
    <main className="main">
      <header><div><h1>{page==='ai'?'AI Prompt Studio':page==='flow'?'Vận hành Flow':page==='license'?'License & Đăng ký':page==='payment'?'Thanh toán':page==='post'?'Hậu kì video':'FLOW AUTO VEO 3'}</h1><p>FLOW AUTO VEO 3 Modern UI</p></div><div className="status">{activity}</div></header>
      ...
      {page==='payment'&&<div className="grid"><Card title="Thanh toán 1 tháng" icon={<CreditCard/>}><img className="qr" src="assets/subscription_qr.png"/><div className="pricing"><div><b>100.000 VND</b><span>/ 1 tháng</span></div></div><p>Nội dung chuyển khoản: <b>FLOWAUTO 1 THANG + SĐT</b><br/>Sau khi thanh toán, gửi Machine ID cho admin để nhận key.</p></Card><Card title="Thanh toán vĩnh viễn" icon={<CreditCard/>}><img className="qr" src="assets/subscription_qr_bank.png"/><div className="pricing"><div><b>1.200.000 VNĐ</b><span>/ vĩnh viễn</span></div></div><p>Nội dung chuyển khoản: <b>FLOWAUTO VINH VIEN + SĐT</b><br/>Zalo: 0989139295<br/>Telegram: https://t.me/flowautotool</p></Card></div>}
