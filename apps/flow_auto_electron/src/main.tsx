...
function App(){
  ...
  return <div className="app">{bootLoading&&<div className="boot-loading"><div className="loader-card"><div className="spinner"></div><b>Đang tải ứng dụng...</b><span>FLOW AUTO VEO 3 đang khởi động, vui lòng chờ.</span></div></div>}{langNotice&&<div className="modal-backdrop"><div className="small-modal"><b>{T('Đã đổi ngôn ngữ','Language changed')}</b><p>{T('Vui lòng khởi động lại app để áp dụng đầy đủ cài đặt ngôn ngữ.','Please restart the app to fully apply the language setting.')}</p><Button variant="primary" onClick={()=>setLangNotice(false)}>OK</Button></div></div>}
    <aside className="side"><div className="brand"><Bot/><div><b>FLOW AUTO VEO 3</b><span>Modern UI</span></div></div><div className="lang-switch"><button className={lang==='VI'?'active':''} onClick={()=>switchLang('VI')}>VI</button><button className={lang==='EN'?'active':''} onClick={()=>switchLang('EN')}>EN</button></div>{nav.map(([id,label,Icon]:any)=><button key={id} onClick={()=>setPage(id)} className={page===id?'active':''}><Icon size={18}/>{label}</button>)}<div className="price">100K / tháng<br/>1.200K vĩnh viễn</div></aside>
    <main className="main">
      <header><div><h1>{page==='ai'?'AI Prompt Studio':page==='flow'?T('Vận hành Flow','Flow Operation'):page==='license'?T('License & Đăng ký','License & Activation'):page==='payment'?T('Thanh toán','Payment'):page==='post'?T('Hậu kì video','Video Post-production'):'FLOW AUTO VEO 3'}</h1><p>FLOW AUTO VEO 3 Modern UI</p></div><div className="status">{activity}</div></header>
      ...
      {page==='payment'&&<div className="payment-page"><Card title={T('Nội dung thanh toán','Payment information')} icon={<CreditCard/>}><div className="pay-info"><div className="pay-block"><h3>VND</h3><p><b>{T('Gói tháng','Monthly')}:</b> 100.000 VND / tháng<br/><b>{T('Vĩnh viễn','Lifetime')}:</b> 1.200.000 VNĐ</p><p>{T('Nội dung CK tháng','Monthly transfer note')}: <b>FLOWAUTO 1 THANG + SĐT</b><br/>{T('Nội dung CK vĩnh viễn','Lifetime transfer note')}: <b>FLOWAUTO VINH VIEN + SĐT</b></p></div><div className="pay-block"><h3>USDT</h3><p><b>{T('Gói tháng','Monthly')}:</b> 5 USDT / tháng<br/><b>{T('Vĩnh viễn','Lifetime')}:</b> 50 USDT</p><p><b>{T('Ví USDT mạng ETH','USDT wallet on ETH network')}:</b><br/><code>0xcbcf357d5d2f5165c544d0ba1d520dbaaaef11c7</code></p></div><div className="pay-contact"><b>{T('Hỗ trợ cấp key','Key/support contact')}</b><br/>Zalo: 0989139295<br/>Telegram: https://t.me/flowautotool<br/><span>{T('Sau khi thanh toán, gửi Machine ID cho admin để nhận key.','After payment, send Machine ID to admin to receive your key.')}</span></div></div></Card><div className="qr-row"><Card title="QR VND" icon={<CreditCard/>}><img className="qr qr-big" src="assets/subscription_qr.png"/></Card><Card title="QR USDT" icon={<CreditCard/>}><img className="qr qr-big" src="assets/subscription_qr_bank.png"/></Card></div></div>}
