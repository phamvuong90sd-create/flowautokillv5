...
function characterSystem(style,media){ return `Bạn là chuyên gia tạo prompt video/ảnh bằng tiếng Việt. Nếu có ảnh nhân vật tham chiếu, BẮT BUỘC phân tích kỹ khuôn mặt, kiểu tóc, màu tóc, vóc dáng, tuổi, giới tính, trang phục, phụ kiện, màu sắc, biểu cảm và phong cách tổng thể. Prompt đầu ra phải bằng TIẾNG VIỆT, mô tả nhân vật tương đồng tối đa với ảnh tham chiếu và giữ nhất quán giữa các cảnh. Không được đổi nhân vật, không đổi giới tính, không đổi khuôn mặt, không đổi trang phục chính nếu người dùng không yêu cầu. Không hứa 100% tuyệt đối; hãy viết theo hướng giữ tương đồng cao nhất/99% theo ảnh tham chiếu. Style: ${STYLE_SUFFIX[style]||''}. Media: ${media}.`; }
...
async function generatePromptsJs(payload){ const apiKey=payload.apiKey||''; const style=payload.style||'CINEMATIC'; const media=payload.mediaType||'IMAGE'; const sys=characterSystem(style,media); const imgs=imageParts(payload.characterImages); const results=[]; for(const idea of splitIdeas(payload.ideas)){ const prompt=await geminiText(apiKey,[...imgs,{text:`Nội dung cần tạo prompt: ${idea}\nYêu cầu: Prompt bằng tiếng Việt, bám sát nội dung và nếu có ảnh thì nhân vật phải tương đồng tối đa với ảnh tham chiếu.`}],sys,false); results.push(prompt); } return {ok:true,generated:writeGenerated('electron-ai-generated-prompts.txt',results)}; }
...
async function generateScriptJs(payload){ const n=durationScenes(payload.duration); const sys=characterSystem(payload.style,'VIDEO')+`\nTạo JSON kịch bản video gồm đúng ${n} scenes. Mỗi scene có sceneNumber,duration,description,prompt bằng TIẾNG VIỆT thật chi tiết. Chỉ trả JSON {title,characterSheet,scenes:[...]}. characterSheet phải mô tả nhân vật theo ảnh tham chiếu, nhấn mạnh giữ tương đồng 99% và nhất quán toàn bộ cảnh.`; const imgs=imageParts(payload.characterImages); const txt=await geminiText(payload.apiKey,[...imgs,{text:`Chủ đề/nội dung: ${payload.topic}. Tổng cảnh: ${n}. Yêu cầu prompt tiếng Việt, nhân vật phải giống ảnh tham chiếu tối đa và đúng nội dung tương ứng.`}],sys,true); const obj=JSON.parse(txt.replace(/^```json\s*|```$/g,'')); const prompts=(obj.scenes||[]).sort((a,b)=>(a.sceneNumber||0)-(b.sceneNumber||0)).map(s=>s.prompt).filter(Boolean); const generated=writeGenerated('electron-ai-script-prompts.txt',prompts); const scriptFile=writeScriptText(obj); return {ok:true,generated,scriptFile}; }
...
function startRunner(payload){
  ...
  const profiles=Array.isArray(payload.profiles)?payload.profiles.filter(x=>x&&String(x.script||x.prompts||'').trim()).slice(0,5):[];
  ...
  let threadFiles=[];
  if(profiles.length){
    threadFiles=profiles.map((pr,i)=>writeThreadPromptFile(`profile-${i+1}.txt`,i,String(pr.script||pr.prompts||'').split(/\n\s*\n/g).map(x=>x.trim()).filter(Boolean)));
  }else{
  ...
  const py=ensurePythonEnv(); const pids=[];
  threadFiles.forEach((pf,idx)=>{
    ...
    const p=spawn(py, [path.join(SCRIPTS_DIR,args[0]), ...args.slice(1)], spawnOpts({detached:true, stdio:['ignore',out,out]})); p.unref(); pids.push(p.pid); fs.writeFileSync(path.join(JOB_DIR,`electron-runner-${idx+1}.pid`),String(p.pid));
  });
  fs.writeFileSync(PID_RUN,String(pids[0]||'')); return {ok:true,pid:pids[0],pids,threads:threadFiles.length,promptFile,runner:'python-stable-hidden-multitab'};
}
